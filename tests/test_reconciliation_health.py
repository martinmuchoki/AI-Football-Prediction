...
def test_uninitialized_reconciliation_blocks_prediction_locking(
    session,
    monkeypatch,
):
    monkeypatch.setattr(
...
def test_warn_below_99_percent_blocks_like_market_refresh(
    session,
    monkeypatch,
):
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: {
            ...
            "fixture_agreement_rate": 0.944737,
            "quality_gate": "WARN",
            "last_checked_at":
                "2026-09-09T23:30:00+00:00",
            "final_holdout_touched": False,
    ...
    assert result["fixture_agreement_rate"] == 0.944737
    assert result["final_holdout_touched"] is False
